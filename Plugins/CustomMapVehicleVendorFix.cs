...
{
    [Info("Custom Map Vehicle Vendor Fix", "Ryz0r", "1.0.3")]
    ...
    public class CustomMapVehicleVendorFix : RustPlugin
    {
        ...
        private void OnServerInitialized()
        {
            if (World.Procedural)
            {
                Puts("Procedural Maps should not have this issue. You're good to go!");
                return;
            }
            ...
            foreach (var vendor in vendorEntities)
            {
                ...
                if (vendor.GetVehicleSpawner() != null && vendor.spawnerRef.IsSet()) continue;
